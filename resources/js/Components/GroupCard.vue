...
const gradient = computed(() => getBrandGradient(props.subscriptionSlug));
...
<template>
    ...
    >
        <div>
            <p class="text-lg font-semibold">{{ subscriptionName }}</p>
            <p class="text-sm opacity-80">{{ spotsLeft }} place{{ spotsLeft > 1 ? 's' : '' }} restante{{ spotsLeft > 1 ? 's' : '' }}</p>
        </div>
        <div class="mt-8">
